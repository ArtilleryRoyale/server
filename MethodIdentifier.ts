...
export function debugStringToMessage(s: string) : string {
    if (s == "r") return "NETWORK_MESSAGE_ROUND_READY";
    if (s == "s") return "NETWORK_MESSAGE_WAIT_FOR_SWITCH";
    if (s == "t") return "NETWORK_MESSAGE_SWITCH_SUCCESS";
    if (s == "a") return "NETWORK_MESSAGE_START";
    if (s == "g") return "NETWORK_MESSAGE_WAIT_FOR_GAMEOPTION";
    if (s == "m") return "NETWORK_MESSAGE_MAP_READY";
    return s;
}
